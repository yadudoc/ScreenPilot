...
def compute_descript(smile):
    ...
    return np.array(descs).flatten().astype(np.float32) #could run in FP16 UNO , something to think about
...
class ModelInferer():
    ...
    def __call__(self, batch):
        return self.model.predict(batch, batch_size=batch.shape[0])
